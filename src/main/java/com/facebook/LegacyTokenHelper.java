...
final class LegacyTokenHelper {
    ...
    public Bundle load() {
        ...
        for (String next : this.cache.getAll().keySet()) {
            try {
                ...
                String str = TAG;
                Logger.log(loggingBehavior, 5, str, "Error reading cached value for key: '" + next + "' -- " + e2);
                return null;
            }
        }
        return bundle;
    ...
    public void save(Bundle bundle) {
        ...
        for (String str : bundle.keySet()) {
            try {
                ...
                LoggingBehavior loggingBehavior = LoggingBehavior.CACHE;
                String str2 = TAG;
                Logger.log(loggingBehavior, 5, str2, "Error processing value for key: '" + str + "' -- " + e2);
                return;
            }
        }
        edit.apply();
    ...
    private void deserializeKey(String str, Bundle bundle) throws JSONException {
        JSONObject jSONObject = new JSONObject(this.cache.getString(str, ServiceLogger.PLACEHOLDER));
