...
public final class Timing extends ReactContextBaseJavaModule implements LifecycleEventListener, HeadlessJsTaskEventListener {
    ...
    private class IdleCallbackRunnable implements Runnable {
        ...
        public void run() {
            boolean access$1400;
            if (!this.mCancelled) {
                long uptimeMillis = SystemClock.uptimeMillis() - (this.mFrameStartTime / JobManager.NS_PER_MS);
    ...
    public void createTimer(int i, int i2, double d2, boolean z) {
        ...
        if (i2 != 0 || z) {
            Timer timer = new Timer(i, (SystemClock.nanoTime() / JobManager.NS_PER_MS) + max, i2, z);
