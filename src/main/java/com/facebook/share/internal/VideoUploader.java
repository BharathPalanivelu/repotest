        ...
        public void initialize() throws FileNotFoundException {
            try {
                if (Utility.isFileUri(this.videoUri)) {
                    ParcelFileDescriptor open = ParcelFileDescriptor.open(new File(this.videoUri.getPath()), SigType.TLS);
        ...
        public void run() {
            if (!this.uploadContext.isCanceled) {
                try {
                    ...
                    endUploadWithFailure(new FacebookException(VideoUploader.ERROR_UPLOAD, (Throwable) e3));
                }
            } else {
            ...
            if (executeAndWait != null) {
                ...
                if (error != null) {
                    ...
                    try {
                        ...
                        endUploadWithFailure(new FacebookException(VideoUploader.ERROR_BAD_SERVER_RESPONSE, (Throwable) e2));
                    }
                } else {
                    handleError(new FacebookException(VideoUploader.ERROR_BAD_SERVER_RESPONSE));
                }
