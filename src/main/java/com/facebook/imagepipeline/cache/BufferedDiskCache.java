...
public class BufferedDiskCache {
    ...
    public k<Boolean> contains(CacheKey cacheKey) {
    ...
    private k<Boolean> containsAsync(final CacheKey cacheKey) {
        try {
            return k.a(new Callable<Boolean>() {
    ...
    public k<EncodedImage> get(CacheKey cacheKey, AtomicBoolean atomicBoolean) {
        try {
            ...
            k<EncodedImage> async = getAsync(cacheKey, atomicBoolean);
            if (FrescoSystrace.isTracing()) {
    ...
    private k<EncodedImage> getAsync(final CacheKey cacheKey, final AtomicBoolean atomicBoolean) {
        try {
            return k.a(new Callable<EncodedImage>() {
    ...
    public k<Void> remove(final CacheKey cacheKey) {
        Preconditions.checkNotNull(cacheKey);
        this.mStagingArea.remove(cacheKey);
        try {
            return k.a(new Callable<Void>() {
    ...
    private k<EncodedImage> foundPinnedImage(CacheKey cacheKey, EncodedImage encodedImage) {
        FLog.v(TAG, "Found image for %s in staging area", (Object) cacheKey.getUriString());
        this.mImageCacheStatsTracker.onStagingAreaHit(cacheKey);
        return k.a(encodedImage);
    }
