...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
    ...
    private h<EncodedImage, Void> onFinishDiskReads(Consumer<EncodedImage> consumer, ProducerContext producerContext, CacheKey cacheKey) {
        ...
        return new h<EncodedImage, Void>() {
            public Void then(k<EncodedImage> kVar) throws Exception {
                if (PartialDiskCacheProducer.isTaskCancelled(kVar)) {
                    ...
                    listener.onProducerFinishWithFailure(id, PartialDiskCacheProducer.PRODUCER_NAME, kVar.g(), (Map<String, String>) null);
                    ...
                    if (f2 != null) {
                        ...
                        String str = id;
                        producerListener.onProducerFinishWithSuccess(str, PartialDiskCacheProducer.PRODUCER_NAME, PartialDiskCacheProducer.getExtraMap(producerListener, str, true, f2.getSize()));
                        BytesRange max = BytesRange.toMax(f2.getSize() - 1);
                        f2.setBytesRange(max);
                        int size = f2.getSize();
                        ...
                        if (max.contains(imageRequest.getBytesRange())) {
                            ...
                            consumer2.onNewResult(f2, 9);
                        } else {
                            consumer2.onNewResult(f2, 8);
                            PartialDiskCacheProducer.this.startInputProducer(consumer2, new SettableProducerContext(ImageRequestBuilder.fromRequest(imageRequest).setBytesRange(BytesRange.from(size - 1)).build(), producerContext2), cacheKey2, f2);
                        }
                    } else {
                        ...
                        PartialDiskCacheProducer.this.startInputProducer(consumer2, producerContext2, cacheKey2, f2);
                    }
                }
                return null;
            }
