package com.shopee.app.database.orm.a;

import com.garena.android.appkit.a.a.a;
import com.j256.ormlite.dao.Dao;
import com.shopee.app.database.orm.bean.DBCheckoutItem;
import java.util.List;
import java.util.concurrent.Callable;

public class k extends a<DBCheckoutItem, Long> {
    public k(com.garena.android.appkit.a.a aVar) {
        super(aVar, DBCheckoutItem.class);
    }

    public void a(final List<DBCheckoutItem> list) {
        try {
            final Dao b2 = b();
            b2.callBatchTasks(new Callable<Object>() {
                public Object call() throws Exception {
                    for (DBCheckoutItem createOrUpdate : list) {
                        b2.createOrUpdate(createOrUpdate);
                    }
                    return null;
                }
            });
        } catch (Exception e2) {
            com.garena.android.appkit.d.a.a(e2);
        }
    }

    public DBCheckoutItem a(long j) {
        try {
            return (DBCheckoutItem) b().queryForId(Long.valueOf(j));
        } catch (Exception e2) {
            com.garena.android.appkit.d.a.a(e2);
            return null;
        }
    }

    public void b(long j) {
        try {
            b().deleteById(Long.valueOf(j));
        } catch (Exception e2) {
            com.garena.android.appkit.d.a.a(e2);
        }
    }
}
