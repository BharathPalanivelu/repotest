...
public class PostCoinCanClaimTask extends AbstractInteractor<Data, Callback> {
    ...
    public PostCoinCanClaimTask(Executor executor, CheckInCoinService checkInCoinService) {
        super(executor);
    ...
    public void run(Data data, final Callback callback) {
        final NetworkData<ServerResult<NullEntity>> networkData = Network.get(this.mCheckInCoinService.postCoinCanClaim(data.mSid, data.mRequestBody));
        if (callback != null) {
            notify(new Runnable() {
                public void run() {
