...
public class PostCoinReleaseTask extends AbstractInteractor<Data, Callback> {
    ...
    public PostCoinReleaseTask(Executor executor, CheckInCoinService checkInCoinService) {
        super(executor);
    ...
    public void run(Data data, final Callback callback) {
        final NetworkData<ServerResult<NullEntity>> networkData = Network.get(this.mCheckInCoinService.postCoinRelease(data.mSid, data.mRequestBody));
        if (callback != null) {
            notify(new Runnable() {
                public void run() {
