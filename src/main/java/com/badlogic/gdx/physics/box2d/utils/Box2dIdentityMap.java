...
import com.badlogic.gdx.physics.box2d.utils.Box2dObjectMap;
import com.google.android.gms.maps.model.BitmapDescriptorFactory;
import java.util.Iterator;
import java.util.NoSuchElementException;

public class Box2dIdentityMap<K, V> implements Iterable<Box2dObjectMap.Entry<K, V>> {
    ...
    public Box2dIdentityMap() {
        this(51, 0.8f);
    }

    public Box2dIdentityMap(int i) {
        this(i, 0.8f);
    }

    public Box2dIdentityMap(int i, float f2) {
        if (i >= 0) {
    ...
    public Box2dIdentityMap(Box2dIdentityMap box2dIdentityMap) {
        this((int) Math.floor((double) (((float) box2dIdentityMap.capacity) * box2dIdentityMap.loadFactor)), box2dIdentityMap.loadFactor);
        this.stashSize = box2dIdentityMap.stashSize;
        K[] kArr = box2dIdentityMap.keyTable;
        System.arraycopy(kArr, 0, this.keyTable, 0, kArr.length);
        V[] vArr = box2dIdentityMap.valueTable;
        System.arraycopy(vArr, 0, this.valueTable, 0, vArr.length);
        this.size = box2dIdentityMap.size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            ...
            if (k != null) {
                V v = vArr[i2];
                if (v == null) {
                    if (box2dIdentityMap.get(k, Box2dObjectMap.dummy) != null) {
                        return false;
                    }
                } else if (!v.equals(box2dIdentityMap.get(k))) {
                    return false;
                }
            }
        }
    ...
    public boolean equalsIdentity(Object obj) {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            K k = kArr[i2];
            if (k != null && vArr[i2] != box2dIdentityMap.get(k, Box2dObjectMap.dummy)) {
                return false;
            }
        }
        return true;
    ...
    public Iterator<Box2dObjectMap.Entry<K, V>> iterator() {
        return entries();
    }
    ...
    private static abstract class MapIterator<K, V, I> implements Iterable<I>, Iterator<I> {
        ...
        final Box2dIdentityMap<K, V> map;
        int nextIndex;
        boolean valid = true;

        public MapIterator(Box2dIdentityMap<K, V> box2dIdentityMap) {
            this.map = box2dIdentityMap;
            reset();
        }
        ...
        public void remove() {
            ...
            if (i >= 0) {
                ...
                this.currentIndex = -1;
                Box2dIdentityMap<K, V> box2dIdentityMap = this.map;
                box2dIdentityMap.size--;
                return;
            }
    ...
    public static class Entries<K, V> extends MapIterator<K, V, Box2dObjectMap.Entry<K, V>> {
        private Box2dObjectMap.Entry<K, V> entry = new Box2dObjectMap.Entry<>();
        ...
        public Entries(Box2dIdentityMap<K, V> box2dIdentityMap) {
            super(box2dIdentityMap);
        }

        public Box2dObjectMap.Entry<K, V> next() {
            if (!this.hasNext) {
    ...
    public static class Values<V> extends MapIterator<Object, V, V> {
        ...
        public Values(Box2dIdentityMap<?, V> box2dIdentityMap) {
            super(box2dIdentityMap);
        }
        ...
        public void toArray(Box2dArray<V> box2dArray) {
            while (this.hasNext) {
                box2dArray.add(next());
            }
        }
    ...
    public static class Keys<K> extends MapIterator<K, Object, K> {
        ...
        public Keys(Box2dIdentityMap<K, ?> box2dIdentityMap) {
            super(box2dIdentityMap);
        }
        ...
        public Box2dArray<K> toArray() {
            Box2dArray<K> box2dArray = new Box2dArray<>(true, this.map.size);
            while (this.hasNext) {
                box2dArray.add(next());
            }
            return box2dArray;
